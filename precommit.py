...
def main() -> int:
    ...
    print("Mypy'ing...")
    subprocess.check_call(["mypy", "packagery", "tests", "bin/pypackagery"], cwd=repo_root.as_posix())

    print("Pylint'ing...")
    subprocess.check_call(["pylint", "--rcfile=pylint.rc", "tests", "packagery", "bin"], cwd=repo_root.as_posix())

    print("Pydocstyle'ing...")
    subprocess.check_call(["pydocstyle", "packagery", "bin"], cwd=repo_root.as_posix())
    ...
    subprocess.check_call(
        ["coverage", "run", "--source", "packagery", "-m", "unittest", "discover", "tests"],
        cwd=repo_root.as_posix(),
        env=env)
    ...
    for pth in (repo_root / "packagery").glob("**/*.py"):
